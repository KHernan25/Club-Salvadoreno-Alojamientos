...
async function verifyMySQLConfig() {
  ...
  try {
    ...
    if (databases.length === 0) {
      console.log(
        "   ⚠️  Base de datos no existe - será creada al inicializar",
      );
    } else {
    ...
    if (error.code === "ECONNREFUSED") {
      ...
      console.log(
        `      CREATE DATABASE ${process.env.DB_NAME || "club_salvadoreno_db"};`,
      );
    }
...
async function verifyEmailConfig() {
  ...
  try {
    ...
    if (!config.auth.pass || placeholders.includes(config.auth.pass)) {
      console.log(
        "   ⚠️  Password es un placeholder - no se enviarán correos reales",
      );
