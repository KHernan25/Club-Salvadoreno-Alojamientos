...
async function setupCompleteDatabase() {
  console.log(
    "🚀 Iniciando configuración COMPLETA de base de datos MySQL...\n",
  );
  ...
  try {
    ...
    const baseScript = fs.readFileSync(
      path.join(__dirname, "setup-mysql-database.sql"),
      "utf8",
    );
    ...
    const accommodationsScript = fs.readFileSync(
      path.join(__dirname, "real-accommodations-data.sql"),
      "utf8",
    );
    ...
    const [totalAccommodations] = await connection.query(
    ...
    console.log(
      `│  ✅ Alojamientos: ${totalAccommodations[0].total.toString().padEnd(23)} │`,
    );
...
async function main() {
  try {
    ...
    if (!prereqsOk) {
      ...
      console.log(
        "   2. Los archivos setup-mysql-database.sql y real-accommodations-data.sql existen",
      );
