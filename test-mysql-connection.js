...
async function testConnection() {
  ...
  try {
    ...
    const [databases] = await connection.query(
      "SHOW DATABASES LIKE 'club_salvadoreno_db'",
    );
    ...
    if (databases.length > 0) {
      ...
      if (tables.length > 0) {
        ...
        try {
          ...
          console.log(
            `🏠 Alojamientos en la base de datos: ${accommodations[0].count}`,
          );
        } catch (err) {
      ...
      console.log(
        "💡 Ejecuta el script de configuración: node setup-database.js",
      );
    }
